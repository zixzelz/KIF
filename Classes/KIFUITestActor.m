...
- (void) selectPickerValue:(NSArray*)pickerColumnValues pickerType:(KIFPickerType)pickerType {
    ...
    [self runBlock:^KIFTestStepResult(NSError **error) {
        ...
        KIFTestStepResult result = [self selectPickerValue:pickerColumnValues pickerView:pickerView error:error];
...
- (void) selectPickerValue:(NSArray*)pickerColumnValues inPickerViewWithAccessibilityIdentifier:(NSString *)identifier {
    ...
    [self waitForAccessibilityElement:NULL view:&pickerView withIdentifier:identifier tappable:NO];
    pickerView = [[pickerView subviewsWithClassNameOrSuperClassNamePrefix:@"UIPickerView"] lastObject];
...
- (KIFTestStepResult)selectPickerValue:(NSArray*)pickerColumnValues pickerView:(UIPickerView*)pickerView error:(NSError **)error {
...
- (void)removeRowAtIndexPath:(NSIndexPath *)indexPath inTableViewWithAccessibilityIdentifier:(NSString *)identifier {
    ...
    if (!deleteElement) {
        [self failWithError:[NSError KIFErrorWithFormat: @"Failed to find accessibility element with the label \"%@\"", accessibilityLabel] stopTest:YES];
    }
